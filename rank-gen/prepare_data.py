    ...
    for example, tkn_example in tqdm(zip(dataset, tkn_dataset)):
        ...
        for turn_id, (turn, tkn_turn) in enumerate(zip(example['dialog'], tkn_example[1:])):
            ...
            if turn['speaker'] == 'wizard' and turn_id > 0 and turn['selected_query']:
                ...
                query_2 = ' '.join([tkn_example[1:][turn_id]['tokenized_text']] +
                                   [mention['spot'] for mention in tkn_example[1:][turn_id]['mentions']
                                    if mention['lp'] == 100])  # coreference
                ...
                if raw_passages:
                    turn['bm25'] = tkn_fn(get_top_n(query_2, get_top_n(query_1, raw_passages, topk), topk))
                if selected_passages:
                    turn['pred_bm25'] = tkn_fn(get_top_n(query_2, get_top_n(query_1, selected_passages, topk),
                                                           topk))  # resorted using response tokens
...
if __name__ == '__main__':
    ...
    # main('../test_rage.json', 'data/test_twtc.json', 'data/tokenized_summary.json', '../test_d20_q1_4p.json', tokenizer, 20, 1)
    # main('../valid_rage.json', 'data/valid_twtc.json', 'data/tokenized_summary.json', '../valid_d20_q1_4p.json', tokenizer, 20, 1)
    # main('../train_rage.json', 'data/train_twtc.json', 'data/tokenized_summary.json', '../train_d20_q1_4p.json', tokenizer, 20, 1)