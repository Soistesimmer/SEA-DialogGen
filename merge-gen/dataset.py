...
class DialogSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        dial_token_id, know_token_id = self.additional_special_tokens['<dialog>'], self.additional_special_tokens['<knowledge>']
        example = self.dataset[session_id]
        dialog = example['dialog']
        history_token_ids = [dial_token_id]+flat_list([[self.additional_special_tokens['<{}>'.format(turn['speaker'])]]
        ...
        if self.type in dialog[turn_id]:
            for passage in dialog[turn_id][self.type][:self.topk]:
                input_ids_list.append(history_token_ids + [know_token_id]+passage['tokens'][:self.max_kn_len-1])
        if not input_ids_list:
            input_ids_list.append(history_token_ids)
        response_token_ids = ([bos_token_id] + dialog[turn_id]['tokens']+[eos_token_id])[:self.max_length]

        return input_ids_list, response_token_ids
